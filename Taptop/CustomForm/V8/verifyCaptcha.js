export const verifyCaptcha = async (container, config) => {
    ...
    if (typeCaptcha === 'Yandex SmartCaptcha') {
        console.log(123)
        // let result = {
        //     type: 'Yandex SmartCaptcha', 
        //     success: false,
        //     failed: false
        // }

        if (!window.smartCaptcha) {
            console.log('smartCaptcha not find!')
        }

        console.log(config.Captcha?.Yandex?.SiteKay)

        const captchaWidgetId = window.smartCaptcha.render('yandex-captcha', {
            sitekey: config.Captcha?.Yandex?.SiteKay,
